...
subroutine gfn0_sp(nat, ichrg, numbers, posotions, status, energy, grad)
    ...
    call gfn0_singlepoint(nat,numbers,posotions,calculator,energy,grad,pr,iostat=io)
    if (io == 0) then
...
!     !> calculation
!     call gfn0_initialize(nat,numbers,posotions,calculator,print=pr,ichrg=ichrg,iostat=io)
!     call gfn0_singlepoint(nat,numbers,posotions,calculator,energy,grad,pr,iostat=io)
